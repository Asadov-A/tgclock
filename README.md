...
On December 31 and January 1 it adds `🎄✨`, on February 14 `❤️🌹`, and on June 1 `🎈☀️`. During the configured Ramadan date range it adds `🌙✨`. Ramadan dates change every year, so update `RAMADAN_START` and `RAMADAN_END` annually.
